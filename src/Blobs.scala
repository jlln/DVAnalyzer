...
object Blobs{
  ...
  def analyzePixelArray(pixels:Array[Array[Int]],colour:String):Result = {
    ...
    ij.IJ.run(image,"Make Binary", "method=Default background=Default stack ")
    image.show()
    ...
    if (areas_index== -1){
      val result_entries = labels.map(l=>new ResultEntry(l,None)).updated(0,new ResultEntry("BlobCount",Some(0)))
      ...
      val slice_centre_x:Float = image_width/2f
      val slice_centre_y:Float = image_height/2f
